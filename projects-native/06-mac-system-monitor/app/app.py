"""
Mac System Monitor — Project 6
Exposes real-time macOS system metrics as Prometheus metrics, using the
cross-platform `psutil` library instead of reading kernel files directly.

Why psutil instead of /proc?
  macOS has NO /proc filesystem — the trick Project 4 uses on Linux does not
  exist here. macOS exposes the same information through the `sysctl` / Mach
  kernel APIs. `psutil` wraps those APIs (and the Linux /proc files) behind one
  identical Python interface, so the exact same code runs on macOS and Linux.
  That is the lesson of this project: a portable library lets you write one
  collector for every OS.
...
# A label so the same dashboard can tell mac from linux if both are scraped.
HOST_OS = "macos"
PORT = 8086
...
@app.route("/")
def index():
    return jsonify({"service": "mac-system-monitor", "status": "ok"})
...
if __name__ == "__main__":
    print(f"Mac System Monitor running on http://0.0.0.0:{PORT}")
