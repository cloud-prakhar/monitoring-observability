# Clean Up the Native WSL2 Setup
...
config, data, and the dedicated service users, leaving WSL2 in the state it was in before you
started.
...
If you set up the systemd service (Step 8a in the install guide):
...
If you ran it manually with `nohup` instead (Step 8b):
...
> A "user does not exist" message here is fine — it just means you ran the native (Step 8b) path
> and never created the user.
...
Grafana was installed from the APT repository, so APT removes it cleanly.
...
`purge` (not just `remove`) also deletes the package's config files. `autoremove` cleans up any
dependencies that were pulled in only for Grafana.

### Step 3 — Remove leftover data and the dedicated user

`purge` removes the program but intentionally leaves your data directory in case you wanted to keep
dashboards. For a fully clean state, remove it too:
...
> The APT package created the `grafana` system user; `userdel` removes it. The `|| true` keeps the
> command from erroring if the user is already gone.

### Step 4 — Remove the APT repository and signing key

So future `apt-get update` runs don't keep contacting Grafana's servers:
...
Prometheus and Grafana are fully removed from WSL2. To start over, follow
